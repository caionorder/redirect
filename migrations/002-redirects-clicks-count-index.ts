...
async function up() {
  ...
  try {
    ...
    const col = db.collection('redirects_clicks');

    await col.createIndex(
      { count: -1 },
      { background: true, name: 'count_desc' }
    );
    console.log('OK indice count_desc');

    console.log('OK migrations/002-redirects-clicks-count-index');
  } finally {
    await client.close();
  }
}
